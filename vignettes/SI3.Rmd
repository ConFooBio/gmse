...
load("../notebook/ms/data/SI3-cull_sim.rda"); # Avoids long compiling time
sim_1 <- sim_cull;
```
...
```{r, echo = FALSE, eval = TRUE}
load("../notebook/ms/data/SI3-scare_sim.rda"); # Avoids long compiling time
sim_2 <- sim_scare;
```
...
Within the default sub-models of GMSE, there are several non-default options that can be set using arguments to `gmse` and `gmse_apply`. Brief explanations of these non-default options appear in the [GMSE documentation](https://cran.r-project.org/web/packages/GMSE/GMSE.pdf) (also [listed](https://confoobio.github.io/gmse/reference/gmse.html) on the [GMSE website](https://confoobio.github.io/gmse/)). Here we further explain some of the less obvious or less intuitive options available in GMSE v0.4.0.7, for which additional explanation beyond the GMSE documentation may be warranted for application to case studies such as the Taiga Bean Geese case study example demonstrated above. Future versions of GMSE are expected to expand upon these options.

**res_move_type**: Resources in default GMSE v0.4.0.7 sub-models can move on the landscape in one of three ways. These different movement types are listed below, and are taken as arguments to `gmse` (e.g., `gmse(res_move_type = 2)`).
