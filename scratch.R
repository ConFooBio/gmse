...
## Find the cost that is the lowest -- maybe use min() from R?
...
for(agent in 1:dim(population)[1]){
    c1 <- runif(n=1);
    if(c1 < cross_prob){
        cross_with                <- dim(population)[1] * floor(runif(n=1)) + 1;
...
winners <- matrix(data = 0, nrow = dim(population)[1], ncol=dim(population)[2]);
...
mean_fitness <- c(mean_fitness, mean(winners[,16]));
