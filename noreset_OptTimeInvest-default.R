...
OTI_stats <- function(df, ts, omit.extinction = FALSE) {
  ...
  sd_ci <- boot_sd_ci(sub$SumSqDev/sub$final_ts, itr = nbs)
  res_str <- c(res_str, mean(sub$SumSqDev/sub$final_ts), sd_ci[1], sd_ci[2], sd_ci[3])
...
## Example
# import data
tab_OTI_default_results <- read.csv("~/Desktop/PhD/GitKraken/gmse_fork_RQ1/modified_calc_cumu-dev/SSD-MEMORY-merged.csv")
...
# example
plot_ATI_diag(upd_thr = 0.1, bud_bon = 0.2)
plot_ATI_diag(upd_thr = 0.1, bud_bon = 0.8)
plot_ATI_diag(upd_thr = 0.3, bud_bon = 0.2)
plot_ATI_diag(upd_thr = 0.3, bud_bon = 0.8)
