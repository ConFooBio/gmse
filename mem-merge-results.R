path <- "~/Desktop/PhD/GitKraken/gmse_fork_RQ1/mem_test_results/"
setwd(dir = path)
...
at0 <- grep(pattern = c("AT0pt0_"), x = content, fixed = T, value = T)
...
from0pt1 <- content[-grep(pattern = c("AT0pt0_"), x = content, fixed = T, value = F)]
...
# export table
write.csv(tab, file = paste(path, "mem-ATI-sim-merged-results.csv", sep = ""))
...
# export table
write.csv(pop, file = paste(path, "mem-pop-ATI-res-merged.csv", sep = ""))
...
# export table
write.csv(cos, file = paste(path, "mem-cos-ATI-res-merged.csv", sep = ""))
...
# export table
write.csv(act, file = paste(path, "mem-act-ATI-res-merged.csv", sep = ""))
...
# export table
write.csv(bgt, file = paste(path, "mem-bgt-ATI-res-merged.csv", sep = ""))
