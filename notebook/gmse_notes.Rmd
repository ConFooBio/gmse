...
[Project updates](#updates)
 ...
 - **2017**
     ...
     - **FEB** [22](#u-22-FEB-2017), [17](#u-17-FEB-2017), [16](#u-16-FEB-2017), [13](#u-13-FEB-2017), [8](#u-8-FEB-2017), [7](#u-7-FEB-2017), [6](#u-6-FEB-2017), [3](#u-3-FEB-2017), [2](#u-2-FEB-2017), [1](#u-1-FEB-2017)
...
**Two forward-looking talks**

Within the next six weeks, I will deliver one 10-12 minute talk and one 15-20 miinute talk. Topics of discussion will include key pieces of the potential perspective outlined above, with a long-term focus toward point 5. Several slides are already prepared with key goals of each slide including:

 - A series of slides showing the logic of genetic algorithms, dynamically simulating one round of a genetic algorithm and its output at each time step (2 minutes, maybe?). 
 - A demonstration of iterated prisoner's dilemma (or any payoff matrix), output including evolved strategies and mean fitness tracked over generations (2 minutes?), dynamically simulated.
 - A sample scenario showing how empirical data for parameterising strategies might be obtained and the used to compare evolved (rational) strategies with real-world decisions (2 minutes?). 
 
I think the above sets of slides are engaging, and useful for the intended audience, and that they fit into a general story that leads to the conclusion that a game-theoretic method of understanding conflict and cooperation in social-ecological systems will be useful, but how to pitch the whole idea in 10-20 minutes needs a bit more fleshing out. The overall goal is to go from the *need to understand conflict and cooperation in conservation* $\to$ *application of game theory, through the use of genetic algorithms, will address this need*, and will **ultimately allow us to simulate management *in silico* in social-ecological systems**. A presentation structure could be as follows:

 - Introduce the idea that conservation conflicts are complex (conflict itself needs no lengthy introduction, as it's the focus of both workshops) -- often it's difficult to understand what the interests of stake-holders are, and how those stake-holders might be expected to make decisions in response to uncertainty and changes in policy (perhaps emphasise examples from earlier talks).
 - Point out that there is no general theory or modelling that is applied to these kinds of conservation conflicts; a common framework for understanding and predicting conflict is needed.
 - Fortunately, there **is** a general theory for understanding conflict and cooperation in other contexts -- namely, evolutionary biology, political science, sport, and economics. Game theory has been the key to addressing the logic of interactions in these fields for decades!


<!--- A useful possiblility might be some sort of statistical analysis during simulations that correlates history and actions into some --->
...
Following up on Saro's notes, I think it is worth thinking about how stake-holders are expected to respond to policy and events, especially with respect to stochasticity. Given the ordinal payoff values, policy error would seem to be low; whereas if if values are continuous, then it would seem like there would be room for stake-holders to be sensitive to small changes in payoffs. I'm not sure which is more realistic -- leading me to question, **are individuals' realised payoffs relevant for their willingness to cooperate (or change or sustain their behaviour, more generally), or is it only the policy that actually matters?**

In other words, assume that some management strategy $M_{1}$ is made that appears fair and favourable to both farmers and conservationists. But also assume that there is a lot of error around the implementation of this policy such that it often results in low payoffs for both stake-holders. What I'm getting at is, I think, *how do the ordinal preferences of policy options map to the real interests of stake-holders?* The most baseline interest of farmers, for example, I suspect would be net income -- the rank order of preferences *only* reflects an accurate payoff matrix insofar as it reflects the order of expected net income from farming. If this mapping of income to rank order changes, so will the payoff matrix.
...
**Now using GitHub projects in development**
...
Darwen, P. J., & Yao, X. (1995). On evolving robust strategies for iterated prisoner’s dilemma. In Progress in Evolutionary Computation (Vol. 956, pp. 276–292). http://doi.org/10.1007/3-540-60154-6_61
