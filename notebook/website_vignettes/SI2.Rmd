...
Note that in the above output, the resource abundance has increased and is now `r sim_4$basic_output$resource_results`. But if we look at `sim_4$X`, the value is still `r sim_4$X`.
...
Unlike with the `gmse` function, `gmse_apply` does not automatically assume that crop production should be replenished after a single time step. The second layer of [the landscape](https://confoobio.github.io/gmse/articles/SI7.html#6-land) holds crop production on a cell. This will be depleted if resources consume crops on the landscape.

```{r}
sim_consume <- gmse_apply(land_dim_1 = 8, land_dim_2 = 8, 
                          res_consume = 0.02, get_res = "Full");
print(round(sim$LAND[,,2], digits = 2));
```
