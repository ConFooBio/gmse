...
Table 1: Select parameter values for initialising generalised management strategy evaluation simulations. See [below](#nondefaults) for explanation of non-default values.
...
Non-default options in default GMSE sub-models {#nondefault}
--------------------------------------------------------------------------------

Within the default submodels of GMSE, there are several non-default options that can be set using arguments to `gmse` and `gmse_apply`. Brief explanations of these non-default options appear in the [GMSE documentation](https://cran.r-project.org/web/packages/GMSE/GMSE.pdf) (also [listed](https://confoobio.github.io/gmse/reference/gmse.html) on the [GMSE website](https://confoobio.github.io/gmse/)). Here we further explain some of the less obvious or less intuitive options available in GMSE v0.4.0.6, for which additional explanation beyond the GMSE documentation may be warranted. Future versions of GMSE are expected to expand upon these options.

**res_move_type**: Resources in default GMSE v0.4.0.7 sub-models can move in one of three ways. These different movement types are listed below, and are taken as arguments to `gmse` (e.g., `gmse(res_move_type = 2)`).

  0. *No movement*. Given this option, resources do not move at all in the default resource sub-model, and instead remain on the location at which they are initialised.
...
**observe_type**: Observation of resources in the observation model can occur in one of four ways.
