...
- `scaring` is assumed to be nonlethal and therefore have an effect of zero on the total abundance of a resource.
- `culling` is assumed to have an effect of $-\sigma(1 + \lambda)$, where $\lambda$ is the GMSE argument `lambda` that defines the baseline population growth rate of resources.
- `castration` is assumed to have an effect of $-\sigma\lambda$.
- `feeding` is assumed to have an effect of $\lambda$.
- `help_offspring` is assumed to have an effect of $\sigma$.
...
The previous section described the fitness function applied when individual's fitness was evaluated for managers; here we explain a separate fitness function that is applied when individuals are instead evaluated for users. Individual fitness as calculated for users ($F_{i}^{u}$) is affected by a user's utility for resources and landscape output (i.e., the predicted summed values of the landscape cells that they own), and the project change in each caused by the user's actions (i.e., the contents of their `POPULATION` layer, specifically rows 1 and 2).
