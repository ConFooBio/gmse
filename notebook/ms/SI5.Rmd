...
The above output from `sim_sum` reports the recruitment (resource or operational model), recruitment estimate (observation error model), management (harvest control model), and user (implementation model) simulation results. This example simulation demonstrates the ability of GMSE to integrate with fisheries libraries such as [FLR](http://www.flr-project.org/) through `gmse_apply`. In addition to being a useful wrapping function for MSE submodels, `gmse_apply` can therefore be used to take advantage of the genetic algorithm in the GMSE default manager and user models. This flexibility will be retained in future versions of `gmse_apply`, allowing custom resource and observation models that are built for specific systems to be integrated with an increasingly complex genetic algorithm simulating various aspects of human decision-making. 
