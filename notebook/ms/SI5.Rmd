...
Diagnostic plots, identical to those of the [modelling stock-recruitment tutorial](http://www.flr-project.org/doc/Modelling_stock_recruitment_with_FLSR.html) for the `nsher_ri` example, are shown below. We note that these plots are made using the `FLCore` package, and are not produced by, nor available in, the `GMSE` package.
