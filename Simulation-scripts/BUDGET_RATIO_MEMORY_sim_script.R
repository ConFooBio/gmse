...
ATI_replicate <- function(UTrange = 0, BBrange = 0,
                          ...
                          ldim = 200,
                          out_file = "BGT-RATIO-MEMORY-results.csv") {
  ...
  columns <- c("rep", "mem", "budget", "ratio", "at", "bb", "extinct", "act_dev", "abs_act_dev", "fin_yield", "max_diff_yield", "inac_ts", "SAD", "final_ts") #, "overK", "param_set"
  ...
  if (at == 0) {
    ...
    for (k in 1:rep) {
      ...
      sim <- gmse(time_max = ts, mem_prv_observ = TRUE, bgt_bonus_reset = FALSE, 
                  ...
                  stakeholders = stkh, scaring = FALSE, manager_budget = ratio*bdgt, user_budget = bdgt, 
      ...
      # Budget ratio
      OTI_default_results[k,4] <- ratio
      
      # AT value
      OTI_default_results[k,5] <- 0
      
      # BB value
      OTI_default_results[k,6] <- 0
      
      # Has extinction occured? (yes = 1, no = 0)
      OTI_default_results[k,7] <- ifelse(final_ts < dim(sim$paras)[1], 1, 0)
      
      ## save costs, actions and population in flw structures
      if (k %% freq == 0) {
        print("Saving budget, costs, actions, actual and observed pop")
        para <- OTI_default_results[k,2:7]
      ...
      if (OTI_default_results[k,7] != 0) {
        
        # Resource actual pop deviation from target
        OTI_default_results[k,8] <- dim(sim$resource[[final_ts-1]])[1]/sim$action[[1]][1,5,1] - 1
        
        # Absolute value
        OTI_default_results[k,9] <- abs(OTI_default_results[k,8])
        
        # Users total final yield
        OTI_default_results[k,10] <- sum(sim$agents[[final_ts-1]][,16])
        
        # Maximum difference between Users yield
        OTI_default_results[k,11] <- (max(sim$agents[[final_ts-1]][,16]) - min(sim$agents[[final_ts-1]][-1,16]))/max(sim$agents[[final_ts-1]][,16])
        
        # Number of timesteps during which Manager chose not to update policy
        OTI_default_results[k,12] <- (final_ts-sum(sim$paras[,107]))/final_ts
        ...
        OTI_default_results[k,13] <- ssd
        
        # Param set
        OTI_default_results[k,14] <- final_ts
      }
      ...
      # If extinction did not occured
        ...
        # Resource actual pop deviation from target
        OTI_default_results[k,8] <- dim(sim$resource[[final_ts]])[1]/sim$action[[1]][1,5,1] - 1
        ...
        # Users total final yield
        # OTI_default_results[k,10] <- sum(sim$agents[[final_ts-1]][,16])
        OTI_default_results[k,10] <- sum(sim$agents[[final_ts]][,16])
        
        # Maximum difference between Users yield
        # OTI_default_results[k,11] <- (max(sim$agents[[final_ts-1]][,16]) - min(sim$agents[[final_ts-1]][-1,16]))/max(sim$agents[[final_ts-1]][,16])
        OTI_default_results[k,11] <- (max(sim$agents[[final_ts]][,16]) - min(sim$agents[[final_ts]][-1,16]))/max(sim$agents[[final_ts]][,16])
  ...
  # for every other at value
    ...
    for (k in 1:rep) {
      ...
      # AT value
      OTI_default_results[k,5] <- at
      
      # BB value
      OTI_default_results[k,6] <- bb
      ...
      if (OTI_default_results[k,7] != 0) {
        ...
        # Number of timesteps during which Manager chose not to update policy
        OTI_default_results[k,12] <- 1-sum(sim$paras[,107])/final_ts
      ...
      # If extinction did not occured
        ...
        # absolute value
        OTI_default_results[k,9] <- abs(OTI_default_results[k,6])
