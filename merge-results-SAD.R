path <- "~/Desktop/PhD/GitKraken/gmse_fork_RQ1/modified_calc_cumu-dev/"
setwd(dir = path)
...
subcontent <- grep(pattern = c("MEMORY_"), x = content, fixed = T, value = T)

#### sim results ####

# initialize a table with the UT0 
at0 <- grep(pattern = c("UT0pt0"), x = subcontent, fixed = T, value = T)
at0.tab <- grep(pattern = "flw", x = at0, fixed = T, value = T, invert = T)

tab <- read.csv(file = paste(path,at0.tab, sep = ""), sep = "\t", header = F)

tab.names <- c("rep", "budget", "at", "bb", "extinct", "act_dev", "abs_act_dev", "fin_yield", "max_diff_yield", "inac_ts", "SAD", "final_ts")
colnames(tab) <- tab.names

# remove them from content
from0pt1 <- subcontent[-grep(pattern = c("UT0pt0"), x = subcontent, fixed = T, value = F)]
...
# export table
write.csv(tab, file = paste(path, "merged-results-SAD-MEMORY.csv", sep = ""))
...
# export table
write.csv(pop, file = paste(path, "pop-SAD-MEMORY.csv", sep = ""))
...
# export table
write.csv(cos, file = paste(path, "cos-MEMORY-SAD.csv", sep = ""))
...
# export table
write.csv(act, file = paste(path, "act-MEMORY-SAD.csv", sep = ""))
...
# export table
write.csv(bgt, file = paste(path, "bgt-MEMORY-SAD.csv", sep = ""))

######## noreset ########

subcontent <- grep(pattern = c("MEMORY"), x = content, fixed = T, value = T, invert = T)
...
# export table
write.csv(tab, file = paste(path, "merged-results-SAD-NORESET.csv", sep = ""))
...
# export table
write.csv(pop, file = paste(path, "pop-SAD-NORESET.csv", sep = ""))
...
# export table
write.csv(cos, file = paste(path, "cos-NORESET-SAD.csv", sep = ""))
...
# export table
write.csv(act, file = paste(path, "act-NORESET-SAD.csv", sep = ""))
...
# export table
write.csv(bgt, file = paste(path, "bgt-NORESET-SAD.csv", sep = ""))

######## stats ########

brut <- as.data.frame(read.csv("merged-results-SAD-MEMORY.csv"))

stat <- OTI_stats(df = brut, ts = 20, omit.extinction = F) 
woe_stat <- OTI_stats(df = brut, ts = 20, omit.extinction = T)

write.csv(stat, file = "stats-MEMORY-SAD.csv")
write.csv(woe_stat, file = "woe-stats-MEMORY-SAD.csv")

brut <- as.data.frame(read.csv("merged-results-SAD-NORESET.csv"))

stat <- OTI_stats(df = brut, ts = 20, omit.extinction = F) 
woe_stat <- OTI_stats(df = brut, ts = 20, omit.extinction = T)

write.csv(stat, file = "stats-NORESET-SAD.csv")
write.csv(woe_stat, file = "woe-stats-NORESET-SAD.csv")
