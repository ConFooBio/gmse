...
<<<<<<< HEAD
    paras <- c(1,                   # 0. The dynamic time step for each function to use 
               ...
               res_birth_type,      # 3. Type of birth (0: none, 1: uniform, 2: Poisson)
               res_death_type,      # 4. Type of death (0: none, 1: uniform, 2: K-based)
               res_birth_K,         # 5. Carrying capacity for birth (-1 = unregulated)
               res_death_K,         # 6. Carrying capacity for death (-1 = unregulated)
               0,                   # 7. The type of AGENT doing the observations
               observe_type,        # 8. The type of observing done for estimating pop.
               1,                   # 9. The type of resource observed (note: dynamic)
               fixed_mark,          # 10. Fix mark? Do observers mark exactly n resources?
               times_observe,       # 11. Times resources observed during one time step
               land_dim_1,          # 12. Land dimension on the x axis
               land_dim_2,          # 13. Land dimension on the y axis
               obs_move_type,       # 14. Agent movement (option same as #2)
               1,                   # 15. Type category for resource observation
               res_min_age,         # 16. Minimum age of sampling (1 excludes juveniles)
               1,                   # 17. Type category for agent observation (default = 1)
               12,                  # 18. Column where res seen recorded in agent array
               res_move_obs,        # 19. Move resources while observing (0/1 = N/Y)
               Euclidean_dist,      # 20. Distance is Euclidean (1) or within-cell (0)
               ga_popsize,          # 21. Population size set in the genetic algorithm
               ga_mingen,           # 22. Minimum number of generations to run a ga
               ga_seedrep,          # 23. Number of replicate agents to seed a ga with
               ga_sampleK,          # 24. The number of sample agents for tournament in ga
               ga_chooseK,          # 25. The number of selected agents in a ga tournament
               ga_mutation,         # 26. The mutation rate of loci in the genetic algorithm
               ga_crossover,        # 27. The crossover rate in the genetic algorithm
               move_agents,         # 28. Move agents once per time step
               max_ages,            # 29. Maximum age of resources
               7,                   # 30. The column of the time trait in the resource array
               11,                  # 31. The column for storing age in the resource array
               RESOURCE_ini,        # 32. The number of resources in the model
               ...
               3,                   # 36. The number of layers (3D depth) in the landscape
               ...
               RESOURCE_ini,        # 61. The number of rows in the observation array             *********
               ttr+times_observe,   # 62. The number of cols in the observation array
               ...
               tend_crop_yld,       # 79. User's improvement of land (proportion)
               ...
               user_res_opts[1],    # 88. Is the movement option available?
               user_res_opts[2],    # 89. Is the killing option available?
               user_res_opts[3],    # 90. Is the castration option available?
               user_res_opts[4],    # 91. Is the feedem option available?
               user_res_opts[5],    # 92. Is the helpem option availabe?
               user_lnd_opts[1],    # 93. Is the kill crop production option available?
               user_lnd_opts[2],    # 94. Is the increase crop growth option available?
               manage_caution,      # 95. How many actions should managers assume exist?
               minimum_cost,        # 96. What is the minimum cost for any action?
               user_budget,         # 97. The user budget
               converge_crit,       # 98. The convergence criteria of the genetic algorithm
               RESOURCE_ini,        # 99. Estimate of res type 1 from the observation model      *******
               lambda,              # 100. Birth rate parameter
               group_think,         # 101. Group think behaviour specified?
               fixed_recapt,        # 102. The number of recaptures in RMR estimation
               land_ownership,      # 103. Is there land ownership among stakeholders
               public_land,         # 104. How much public land is there (proportion)
               action_thres,        # 105. Dev est pop from manager target triggering
               ...
               budget_bonus,        # 110. Budget bonus
               age_repr,            # 111. Age at which individuals can first reproduce
               16,                  # 112. Column in agent array where budget is located 
               manager_budget,      # 113. The manager's budget
               10,                  # 114. Column in resource array where offspring located
               20,                  # 115. Column in resource array where consumed located
               consume_surv,        # 116. Consumption needed for resource to survive
               consume_repr,        # 117. Consumption needed for resource offspring +1
