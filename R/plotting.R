...
plot_gmse_actions <- function(agents, paras, ACTION, COST){
    ...
    for(time in 1:max_time){
        cull_act[time, 1] <- COST[[time]][1, 9, 2];
        for(stakeholder in 2:users){
            cull_act[time, stakeholder] <- ACTION[[time]][1, 9, stakeholder];
        }
        scar_act[time, 1] <- COST[[time]][1, 8, 2];
        for(stakeholder in 2:users){
            scar_act[time, stakeholder] <- ACTION[[time]][1, 8, stakeholder];
        }
        cast_act[time, 1] <- COST[[time]][1, 10, 2];
        for(stakeholder in 2:users){
            cast_act[time, stakeholder] <- ACTION[[time]][1, 10, stakeholder];
        }
        feed_act[time, 1] <- COST[[time]][1, 11, 2];
        for(stakeholder in 2:users){
            feed_act[time, stakeholder] <- ACTION[[time]][1, 11, stakeholder];
        }
        help_act[time, 1] <- COST[[time]][1, 12, 2];
        for(stakeholder in 2:users){
            help_act[time, stakeholder] <- ACTION[[time]][1, 12, stakeholder];
        }
    }
    ...
    if(para_vec[89] == 1){
        y1 <- max(scar_act[,1]);
        y2 <- y1 * 1.2
        plot(x = 1:max_time, y = scar_act[,1], type = "l", lwd = 2, 
             cex.axis = 1.5, xaxt = "n", ylim = c(0, y2));
        par(new = TRUE);
        max_acts <- apply(X = scar_act, MARGIN = 1, FUN = max);
        y3 <- max(max_acts);
        y4 <- y3 * 1.2
        plot(x = 1:max_time, y = max_acts, type = "n", lwd = 2, 
             ylim = c(0, y4), xaxt="n", yaxt="n", 
             cex.axis = 1.5);
        for(stakeholder in 2:users){
           points(x = 1:max_time, y = scar_act[,stakeholder], type = "l", 
                  lwd = 1, col = cols[1]);
        }
        right_x <- floor(seq(from = 1, to = max(max_acts), length.out = 5));
        axis(side = 4, at = right_x[-1], cex.axis = 1.5);
        legend(x = 1, y = y4 + y4*0.1, fill = cols[1:5], horiz = TRUE,
               legend = c("scaring", "culling", "castration", "feeding", 
                          "helping"), bty = "n", cex = 1.5);
        abline(h = y3 + y3*0.1, lwd = 2);
    }else{
        plot(x = 1:max_time, y = scar_act[,1], type = "n", xaxt= "n", 
             yaxt = "n", ylim = c(0, 110));
    ...
    #---- Culling
    if(para_vec[90] == 1){
        plot(x = 1:max_time, y = cull_act[,1], type = "l", lwd = 2, 
             cex.axis = 1.5, xaxt = "n");
        par(new = TRUE);
        max_acts <- apply(X = cull_act, MARGIN = 1, FUN = max);
        plot(x = 1:max_time, y = max_acts, type = "n", lwd = 2, 
             ylim = c(0, max(max_acts)), xaxt="n", yaxt="n", cex.axis = 1.5);
        for(stakeholder in 2:users){
            points(x = 1:max_time, y = cull_act[,stakeholder], type = "l", 
                   lwd = 1, col = cols[2]);
        }
        right_x <- floor(seq(from = 1, to = max(max_acts), length.out = 5));
        axis(side = 4, at = right_x[-1], cex.axis = 1.5);
    ...
    #---- Castrating
    if(para_vec[91] == 1){
        plot(x = 1:max_time, y = cast_act[,1], type = "l", lwd = 2, 
             cex.axis = 1.5, xaxt = "n");
        par(new = TRUE);
        max_acts <- apply(X = cast_act, MARGIN = 1, FUN = max);
        plot(x = 1:max_time, y = max_acts, type = "n", lwd = 2, 
             ylim = c(0, max(max_acts)), xaxt="n", yaxt="n", cex.axis = 1.5);
        for(stakeholder in 2:users){
            points(x = 1:max_time, y = cast_act[,stakeholder], type = "l", 
                   lwd = 1, col = cols[3]);
        }
        right_x <- floor(seq(from = 1, to = max(max_acts), length.out = 5));
        axis(side = 4, at = right_x[-1], cex.axis = 1.5);
    ...
    mtext("Manager policy", side = 2, line = 3.5, cex = 1.5, col = "black");
    mtext("User action", side = 4, line = 3.5, cex = 1.5, col = "black");
    #---- Feeding
    if(para_vec[92] == 1){
        plot(x = 1:max_time, y = feed_act[,1], type = "l", lwd = 2, 
             cex.axis = 1.5, xaxt = "n");
        par(new = TRUE);
        max_acts <- apply(X = feed_act, MARGIN = 1, FUN = max);
        plot(x = 1:max_time, y = max_acts, type = "n", lwd = 2, 
             ylim = c(0, max(max_acts)), xaxt="n", yaxt="n", cex.axis = 1.5);
        for(stakeholder in 2:users){
            points(x = 1:max_time, y = feed_act[,stakeholder], type = "l", 
                   lwd = 1, col = cols[4]);
        }
        right_x <- floor(seq(from = 1, to = max(max_acts), length.out = 5));
        axis(side = 4, at = right_x[-1], cex.axis = 1.5);
    ...
    #---- Helping
    if(para_vec[93] == 1){
        plot(x = 1:max_time, y = help_act[,1], type = "l", lwd = 2, 
             cex.axis = 1.5);
        par(new = TRUE);
        max_acts <- apply(X = help_act, MARGIN = 1, FUN = max);
        plot(x = 1:max_time, y = max_acts, type = "n", lwd = 2, 
             ylim = c(0, max(max_acts)), xaxt="n", yaxt="n", cex.axis = 1.5);
        for(stakeholder in 2:users){
            points(x = 1:max_time, y = help_act[,stakeholder], type = "l", 
                   lwd = 1, col = cols[5]);
        }
        right_x <- floor(seq(from = 1, to = max(max_acts), length.out = 5));
        axis(side = 4, at = right_x[-1], cex.axis = 1.5);
...
plot_gmse_conflict <- function(agents, paras, ACTION, COST){
